...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and not insect.die:
                return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location is not hive:
            if count >= self.min_range and count <= self.max_range and location.bees:
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does nothing but block the paths of bees. WallAnt has a high armor"""
    name = 'Wall'
    implemented = True
    food_cost = 4
    def __init__(self, armour = 4):
        Ant.__init__(self, armour)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower ant is a thrower ant that can be placed on Water"""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.queen_alive:
            self.die = True
        else:
            self.die = False
            QueenAnt.queen_alive = True
        # END Problem 13
        ...
        if self.die:
            ...
            while location is not None:
                if location.ant is not None:
                    if location.ant.is_container and location.ant.contained_ant is not None:
                        if not location.ant.contained_ant.is_doubled:
                            location.ant.contained_ant.damage *= 2
                            location.ant.contained_ant.is_doubled = True
                    elif not location.ant.is_doubled:
                        location.ant.damage *= 2
                        location.ant.is_doubled = True
